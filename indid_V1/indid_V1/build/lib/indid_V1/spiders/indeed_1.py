...
class Indeed1Spider(scrapy.Spider):
    name = 'indeed-DS-update'
    allowed_domains = ['indeed.com']
    
    states=['AK', 'AL', 'AR', 'AS', 'AZ', 'CO', 'CT', 'DC', 'DE', 'FL', 'GA', 'GU', 'HI', 'IA', 'ID', 'IL', 'IN',\
    'KS', 'KY', 'LA', 'MA', 'MD', 'ME', 'MI', 'MN', 'MO', 'MP', 'MS', 'MT', 'NC', 'ND', 'NE', 'NH', 'NJ', 'NM', 'NV', 'NY', 'OH',\
    'OK', 'OR', 'PA', 'PR', 'RI', 'SC', 'SD', 'TN', 'TX', 'UM', 'UT', 'VA', 'VI', 'VT', 'WA', 'WI', 'WV', 'WY']
    
    start_urls = ['https://www.indeed.com/jobs?q=data+scientist&l={}&radius=0&fromage=20&sort=date&limit=50'.format(state) for state in states]
    ...
    def parse_first_result(self,response):
        ...
        if other_result_pgaes_links:
            ...
            for url in urls:
                yield scrapy.Request(url = url,
                                 callback = self.parse_result_page, headers={"User-Agent": "Mozilla/5.0"},
                               meta={"proxy": "http://pxu15333-0:QvfYg4kOWGoW5plrJ$Ax@x.botproxy.net:8080"})
        else:
    ...
    def parse_pages(self,response):
        ...
        job_title = response.xpath('//div[contains(@class,"jobsearch-JobInfoHeader-title")]/h3/text()').extract_first()
