...
class Indeed1Spider(scrapy.Spider):
    name = 'indeed-Data-Analyst-weekly-update-helper'
    ...
    start_urls = ['https://www.indeed.com/jobs?q=data+analyst&l={}&radius=0&fromage=10&sort=date&limit=50&filter=0'.format(state) for state in states]
    ...
    def parse_pages(self,response):
        ...
        if len(date)<2:
            scraped_d = date[0]
        # modification for 2 type of responses from requests
        elif date[1] ==' - ':
            scraped_d = date[2]
        else:
            scraped_d = date[1]
